# runner_safetreasuregoal_compare.py
...
from ppolag import train_ppolag
from safetreasuregoal_ltlppo_labelled_new import train_labelled_ltl_ppo
...
def print_latest(algo, seed, log):
    print(
        ...
        + (
            f" | Lag {latest(log, 'lagrangian'):7.3f}"
            if "lagrangian" in log
            else ""
        )
        + (
            f" | LTLRet {latest(log, 'eval_ltl_return'):8.3f}"
...
def print_summary(results):
    summary_metrics = list(METRICS.keys()) + list(EXTRA_METRICS.keys())
