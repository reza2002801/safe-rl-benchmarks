...
class ObsWrapper:
    ...
    def step(self, action):
        ...
        return self.flatten(obs), reward, terminated, truncated, info
    ...
    def flatten(self, obs):
        ...
        return np.concatenate([pos, flags, treasures])
...
class ActorCriticLag(nn.Module):
    ...
    def forward(self, x):
        ...
        return (
            self.actor(z),
            self.reward_critic(z).squeeze(-1),
            self.cost_critic(z).squeeze(-1),
        )
...
def make_env(seed=0):
    return ObsWrapper(SafeTreasureDoorKeyGrid(seed=seed))

def evaluate(env, model, episodes=30):
    returns, costs, successes, ltls = [], [], [], []
    ...
    for _ in range(episodes):
        ...
        while not done:
            ...
            # has_key and door_open are provided directly in info by the fixed env
...
def train_ppolag(
    ...
    while global_step < total_steps:
        ...
        avg_ep_cost = float(np.mean(ep_costs)) if ep_costs else float(np.sum(cost_arr))
        lagrangian = max(0.0, lagrangian + lag_lr * (avg_ep_cost - cost_limit))

        update += 1

        if update % eval_every_updates == 0:
            eval_env = make_env(seed=seed + 10_000 + update)
    ...
    if save_model:
        torch.save(model.state_dict(), f"ppolag_random_safetreasuregoal_seed{seed}.pt")
